...
.provider('securityAuthorization', {
  ...
  requireOwnerOrAdminUser: function(otherUserId) {
      return ['securityAuthorization', function(securityAuthorization) {
              return securityAuthorization.requireOwnerOrAdminUser(otherUserId);
      }];
  },
  ...
  $get: ['security', 'securityRetryQueue', function(security, queue) {
    var service = {
      ...
      requireOwnerOrAdminUser: function(otherUserId) {
        var promise = security.requestCurrentUser().then(function(userInfo) {
          if ( !security.isAdmin() && !security.isOwner(otherUserId)) {
            return queue.pushRetryFn('unauthorized-client', service.requireAdminUser);
          }
        });
        return promise;
